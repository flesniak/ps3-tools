...
try:
	...
	print "Failed to import pkgcrypt. Falling back to slow crypt implementation."
	print "To fix, run:"
	print ""
...
def pack(folder, contentid, contenttype, outname=None):
	...
	content_types = {
		"gamedata": 0x4, # GameData (also patches) /dev_hdd0/game/
		"gameexec": 0x5, # GameExec /dev_hdd0/game/
		"ps1emu": 0x6, # PS1emu /dev_hdd0/game/
		"psp": 0x7, # PSP & PCEngine /dev_hdd0/game/
		"theme": 0x9, # Theme /dev_hdd0/theme
		"widget": 0xA, # Widget /dev_hdd0/widget
		"license": 0xB, # License /dev_hdd0/home/<current user>/exdata
		"vshmodule": 0xC, # VSHModule /dev_hdd0/vsh/modules/
		"psn": 0xD, # PSN Avatar /dev_hdd0/home/<current user>/psn_avatar
		"pspgo": 0xE, # /dev_hdd0/game/ Displayed as Unknown Album: Corrupted Data
		"minis": 0xF, # /dev_hdd0/game/
		"neogeo": 0x10, # /dev_hdd0/game/
		"vmc": 0x11, # /dev_hdd0/tmp/vmc/
		"ps2classic": 0x12, # Seen on PS2 classic /dev_hdd0/game/
		"seen": 0x14, # Seen on PSP remastered /dev_hdd0/game/
		"psp2gd": 0x15, # PSP2GD (PSVita Game Data)
		"psp2ac": 0x16, # PSP2AC (PSVita Additional Content)
		"psp2la": 0x17, # PSP2LA (PSVita LiveArea)
		"psm": 0x18, # PSVita PSM PlayStation Mobile
		"wt": 0x19, # Web TV /dev_hdd0/game/
		"psm2": 0x1D, # PSVita PSM
		"psp2theme": 0x1F # PSP2Theme (PSVita Theme)
	}
	type_id = content_types[contenttype]
...
def usage():
    ...
    python pkg.py target-directory [out-file]
        -c | --contentid             list packaged files.
        -t | --contenttype          extract package.
...
def main():
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "hx:dvl:c:t:", ["help", "extract=", "debug","version", "list=", "contentid=", "contenttype="])
	...
	if extract:
		...
		if len(args) == 1 and contentid != None:
			pack(args[0], contentid, contenttype)
		elif len(args) == 2 and contentid != None:
			pack(args[0], contentid, contenttype, args[1])
		else:
			usage()
			sys.exit(2)
