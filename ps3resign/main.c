...
char default_act_dat[] = "act.dat";
char default_idps_hex[] = "idps.hex";
struct opts_s {
	int verbose;
	char* outdir;
	char* act_dat;
...
int read_act_dat(uint8_t* act_dat_key, uint64_t* account_id, int forge)
{
	FILE* fp=fopen(opts.act_dat,"rb");
	if (!fp) {
		if (forge) {
			fp=forge_act_dat(opts.act_dat);
			if (!fp)
				return 1;
		} else {
			fprintf(stderr, "Failed to open %s: %s\n", opts.act_dat, strerror(errno));
			return 1;
		}
	}
	if (opts.verbose)
...
int read_file_bytes(const char* path, void* buf, size_t size)
{
...
int read_idps_hex(uint8_t* idps)
{
	return read_file_bytes(opts.idps_hex, idps, 0x10);
}

int read_rap(const char* path, uint8_t* rap)
{
	return read_file_bytes(path, rap, 0x10);
}

int read_act_dat_and_make_rif(const char* path)
{
	...
	uint8_t idps[0x10];
	if (read_idps_hex(idps))
		return 1;

	uint8_t act_dat_key[0x10];
	uint64_t account_id;
	if (read_act_dat(act_dat_key, &account_id, 0))
		return 1;

	uint8_t rap[0x10];
	if (read_rap(path, rap))
		return 1;
...
//	memcpy(rif+0x50, klicensee_enc_rif,0x10);
	...
	char out_path[4096];
	if (opts.outdir) {
		strcpy(out_path, opts.outdir);
		strcat(out_path, "/");
		char* filename = strrchr(path, '/');
		if (filename == 0)
			strcpy(out_path, path);
		else
			strcpy(out_path, filename+1);
	} else
		strcpy(out_path, path);
	strcpy(out_path+strlen(path)-4, ".rif");
...
int read_rif_key(char *content_id, uint8_t *rifkey)
{
	...
	// uint8_t act_dat_key[0x10];
	// err = read_act_dat(act_dat_key, 0, 0);
	// if (err)
	// 	return err;
...
int sign_act_dat()
{
	...
	if (read_file_bytes(opts.act_dat, act_dat_buf, 0x1038))
		return 1;
	...
	strcat(out_path, "signed_act.dat");
	if (opts.verbose)
		printf("writing:%s\n", out_path);
	FILE* fp=fopen(out_path, "wb");
	if (!fp) {
...
int sign_enc(FILE *fp)
{
	...
	if (fseek(fp, 0xD8, SEEK_SET))
		return 1;
	if (!fwrite(R+1, 0x14, 1, fp))
		return 1;
	if (!fwrite(S+1, 0x14, 1, fp))
		return 1;
	free(buf);
	return 0;
}
...
int decrypt_retail_pkg_data(uint8_t *buf, uint64_t size, uint8_t *data_riv, const uint8_t *gpkg_key)
{
...
int parse_psp_pkg(uint8_t *pkg, uint32_t toc_len, uint8_t *iv_const, uint32_t data_size, int file_out, uint64_t offset_data)
{
	...
	for(i=0;i<number_files;i++)
	{
		...
		if (opts.verbose) {
			...
			printf("header[i].file_off=%lx\n", header[i].file_off);
			printf("header[i].file_size=%lx\n", header[i].file_size);
...
int sign_pkg(const char* path)
{
	...
	uint64_t pkg_size=stat1.st_size;
	if (opts.verbose)
		printf("pkg size:%016lx\n", pkg_size);

	FILE* fd_in=fopen(path, "rb");
	if (!fd_in) {
	...
	char out_path[4096];
	strcpy(out_path, path);
	strcpy(out_path+strlen(path)-4, "_signed.pkg");
	int fd_out=open(out_path, O_RDWR|O_CREAT|O_TRUNC);
	if (!fd_out) {
	...
	size_t nread;
	ssize_t nwritten = 0;
	while (nread = fread(buf, 1, block_size, fd_in), nread > 0 && nwritten >= 0)
	{
		uint8_t *out_ptr = buf;
		ssize_t nwritten;
	...
	if (err || nwritten < 0)
	{
		fprintf(stderr, "Failed to copy pkg data: %s\n", strerror(errno));
		close(fd_out);
		return 1;
	}

	if (pkg_size>(10*1024*1024))
	{
		buf=(uint8_t *)mmap(0, 10*1024*1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd_out, 0);
	}
	else
	{
		buf=(uint8_t *)mmap(0, pkg_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd_out, 0);
	}
	...
	const uint8_t pkg_key[0x10]={0x2E,0x7B,0x71,0xD7,0xC9,0xC9,0xA1,0x4E,0xA3,0x22,0x1F,0x18,0x88,0x28,0xB8,0xF8};
	const uint8_t pkg_key_psp[0x10]={0x07,0xF2,0xC6,0x82,0x90,0xB5,0x0D,0x2C,0x33,0x81,0x8D,0x70,0x9B,0x60,0xE6,0x2B};
	const uint8_t retail_flag=0x80;
	if(buf[4]!=retail_flag)
	{
		buf[4]=retail_flag;
		...
		decrypt_debug_pkg(buf, data_size, offset_data, fd_out, pkg_size);
	...
	SHA1_Init( &ctx );
	uint64_t sha1_loop;
	for(sha1_loop=0; sha1_loop<pkg_size-0x20; sha1_loop+=100*1024*1024)
	{
		if(pkg_size-sha1_loop-0x20<100*1024*1024)
		{
			buf=(uint8_t *)mmap(0, pkg_size-sha1_loop, PROT_READ | PROT_WRITE, MAP_SHARED, fd_out, sha1_loop);
			SHA1_Update( &ctx, buf, pkg_size-sha1_loop-0x20 );
			SHA1_Final(buf+pkg_size-sha1_loop-0x20, &ctx);
			munmap(buf, pkg_size-sha1_loop);
		}
		else
		{
	...
	close(fd_out);
	if (opts.verbose)
		printf("signed pkg written to %s\n", out_path);
	return 0;
}

void print_usage(char* app) {
	fprintf(stderr, "Usage: %s [-hv] [-a act.dat] [-i idps.hex] [-o output_dir] <file.pkg|file.edat|file.rap|file.ENC> [file2] [...]\n", app);
	fprintf(stderr, "  -h: display this usage notice\n");
	fprintf(stderr, "  -v: verbose mode\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "When file is *.rap: make RIF and sign act.dat\n");
	fprintf(stderr, "When file is *.edat: resign edat\n");
	fprintf(stderr, "When file is *.pkg: sign PSN/PSP/PSX/Minis pkg\n");
	fprintf(stderr, "When file is *.ENC: sign ISO.BIN.ENC and optionally CONFIG\n");
}

int parse_options(int argc, char** argv, struct opts_s* opts)
{
	memset(opts, 0, sizeof(struct opts_s));
	opts->act_dat = default_act_dat;
	opts->idps_hex = default_idps_hex;

	char c;
	while ((c = getopt(argc, argv, "hvo:a:i:")) != -1)
		switch (c) {
		case 'v':
			opts->verbose++;
			break;
		...
		case 'a':
			...
			break;
		case 'i':
			opts->idps_hex = optarg;
			break;
		case 'h':
			print_usage(argv[0]);
			return 1;
		case '?':
			if (optopt == 'o' || optopt == 'a' || optopt == 'i')
...
{
	printf("----------------PS3XPLOIT RESIGNER v3 MOD------------\n");
	printf("HAN v3 tools brought you by W, escortd3w, bguerville, habib\n");
	printf("Special thanks to Joonie for testing this tool\n");
	printf("-----------------------------------------------------\n");

	int err = parse_options(argc, argv, &opts);
	if(err)
	{
		return err-1;
	}
	...
	char* input = 0;
	bool do_sign_act_dat = false;
	while (input = argv[optind++], optind <= argc && input != 0) {
		if((strstr(input, ".rap")) || (strstr(input, ".RAP")))
		{
			do_sign_act_dat = true;
			err = read_act_dat_and_make_rif(input);
			if (err) {
				fprintf(stderr, "Failed to make rif\n");
				return err;
			}
			printf("made rif for %s\n", input);
		}
		else if((strstr(input, ".EDAT")) || (strstr(input, ".edat")))
		{
		...
		else if((strstr(input, ".pkg")) || (strstr(input, ".PKG")))
		{
			err = sign_pkg(input);
			if (err) {
				fprintf(stderr, "Failed to sign %s\n", input);
		...
		else if(strstr(input, ".ENC"))
		{
			...
			if(fp)
			{
				...
				if (err) {
					fprintf(stderr, "Failed to sign CONFIG\n");
					return err;
				}
				printf("esigned CONFIG\n");
				fclose(fp);
			}
		}
	}
	...
	if (do_sign_act_dat) {
		err = sign_act_dat();
		if (err) {
			fprintf(stderr, "Failed to create signed_act.dat\n");
			return err;
		}
		printf("signed_act.dat created\n");
	}
